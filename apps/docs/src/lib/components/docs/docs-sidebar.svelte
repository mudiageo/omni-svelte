...
<Sidebar.Root variant="inset">
	...
	<Sidebar.Content class="mt-2">
		...
		<div class="px-2 pb-2">
			<button
				class="flex w-full items-center gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm text-muted-foreground hover:bg-muted transition-colors"
			>
				<svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
				</svg>
				<span class="flex-1 text-left text-xs">Search docs...</span>
				<kbd class="pointer-events-none hidden select-none rounded border bg-background px-1.5 py-0.5 font-mono text-[10px] opacity-100 sm:inline-flex">⌘K</kbd>
			</button>
		</div>
		...
		{#each docsNav as section}
			<Collapsible.Root open={isSectionActive(section.items)} class="group/collapsible">
				<Sidebar.Group class="py-0">
					<Sidebar.GroupLabel
						class="group/label flex cursor-pointer items-center text-[11px] uppercase tracking-widest text-muted-foreground/60 hover:text-foreground transition-colors"
						asChild
					>
						{#snippet child({ props })}
									...
									<Sidebar.MenuItem>
										<Sidebar.MenuButton
											...
											class={cn(
												'text-sm h-8',
												isActive(item.href)
													? 'font-medium text-sidebar-primary bg-sidebar-accent'
												...
												<a href={item.href ?? '#'} {...props}>
													...
													{#if item.label}
														<Badge
															...
															class="ml-auto text-[9px] px-1 h-3.5 font-semibold uppercase tracking-wider py-0"
														>
															{item.label}
														</Badge>
													{/if}
	...
	<Sidebar.Footer>
		<Sidebar.Menu>
			<Sidebar.MenuItem>
				<Sidebar.MenuButton href="https://github.com/mudiageo/omni-svelte" target="_blank" rel="noopener">
					{#snippet child({ props })}
						<a href="https://github.com/mudiageo/omni-svelte" target="_blank" rel="noopener" {...props}>
