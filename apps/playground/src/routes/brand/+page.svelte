<script lang="ts">
  ...
  const fonts = [
    { role: 'Display / Headline', family: 'Inter', weight: '700 / 900', example: 'SvelteKit with superpowers.' },
    { role: 'Body', family: 'Inter', weight: '400 / 500', example: 'Transform your SvelteKit app into a production-ready powerhouse.' },
    { role: 'Monospace / Code', family: 'Geist Mono', weight: '400', example: 'omni-svelte/database' }
  ];
...
<main class="brand-page">
  <header class="brand-header">
    ...
    <p class="subtitle">Design system, logos, colour palette, and usage guidelines for omni-svelte.</p>
  </header>

  <!-- LOGOS -->
  <section class="brand-section">
    <h2>Logomark</h2>
    <p class="section-desc">The omni-svelte logomark is based on the Svelte flame silhouette, recoloured in a premium orange → amber/gold gradient. Two variants are provided.</p>
    <div class="logo-grid">
      {#each logos as logo}
        <div class="logo-card">
    ...
    <div class="favicon-row">
      {#each [64, 48, 32, 24, 16] as size}
        <div class="favicon-item">
          <div class="favicon-preview" style="background:#0F0F1A; padding:{size < 24 ? 8 : 12}px; border-radius:8px;">
  ...
  <section class="brand-section">
    ...
    <div class="gradient-bar" style="background: linear-gradient(90deg, #7C2105, #CC3600, #E07B00, #F59E0B)">
    ...
    <div class="type-stack">
      {#each fonts as f}
        <div class="type-row">
          ...
          <div class="type-sample" style="font-family:{f.family === 'Geist Mono' ? 'monospace' : 'sans-serif'}; font-weight:{f.weight.split(' / ')[0]}">
            {f.example}
          </div>
        </div>
      {/each}
...
<style>
  ...
  h1 { font-size: 3rem; font-weight: 900; margin: 0 0 0.5rem; }
  h2 { font-size: 1.5rem; font-weight: 700; margin: 0 0 0.5rem; }
  .subtitle { color: #A1A1C7; font-size: 1.1rem; margin: 0; }
  .section-desc { color: #A1A1C7; margin: 0 0 1.5rem; }
  ...
  .logo-info strong { font-size: 1rem; color: #fff; }
  .logo-info p { font-size: 0.82rem; color: #A1A1C7; margin: 0; }
  ...
  .dl-btn:hover { background: rgba(255,62,0,0.1); }
  ...
  .favicon-item span { font-size: 0.75rem; color: #A1A1C7; }
  ...
  .swatch strong { font-size: 0.85rem; font-weight: 600; }
  .swatch code { font-size: 0.75rem; opacity: 0.85; font-family: monospace; }
  .swatch small { font-size: 0.65rem; opacity: 0.6; }
  ...
  /* Typography */
  .type-stack { display: flex; flex-direction: column; gap: 1.5rem; margin-top: 1.5rem; }
  .type-row {
    display: flex;
    gap: 2rem;
  ...
  .type-meta { min-width: 200px; }
  .type-meta strong { display: block; color: #fff; font-size: 0.9rem; }
  .type-meta span { color: #A1A1C7; font-size: 0.8rem; }
  .type-sample { font-size: 1.1rem; color: #F8F8FF; line-height: 1.5; }

  /* Guidelines */
  .guidelines { list-style: none; padding: 0; display: flex; flex-direction: column; gap: 0.75rem; margin-top: 1rem; }
  .guidelines li { padding: 0.75rem 1rem; border-radius: 8px; font-size: 0.9rem; }
  .guidelines li:has(✅) { background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.2); }
  .guidelines li:has(❌) { background: rgba(255,62,0,0.08); border: 1px solid rgba(255,62,0,0.15); }
</style>
