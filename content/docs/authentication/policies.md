...
Route protection handles _authentication_ — verifying who someone is. Policies handle _authorization_ — deciding what they are allowed to do.
...
export const {
	...
	authorize: async ({ user, operation, input }) => {
		...
		const record =
			(operation === 'update' || operation === 'remove') && recordId
