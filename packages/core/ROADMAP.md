...
- **🔧 Broken, not just incomplete:** `OmniPlugin` is imported from `../types.js` in `plugins/logging` and `plugins/cors`, but no `packages/core/src/plugins/types.ts` exists — the roadmap's "[x] Plugin API types" checkbox should be unchecked until this resolves. This is the single highest-leverage bug fix on the list because the entire Plugin Ecosystem phase (v0.8) depends on it.
...
| `PLUGIN` | `OmniPlugin` lifecycle interface | 🔧 | Needs the missing `types.ts` before anything else can build on it |
...
| First-party SMS drivers for African gateways (Termii, Africa's Talking) | 🆕 | High *(for target market)* | M | new | OTP, transactional SMS for Nigerian/African users where email-only auth underperforms |
...
| First-party Paystack/Flutterwave plugins | 🆕 | High *(for target market)* | L | `PLUGIN` | Makes OmniSvelte the obvious choice for Nigerian/African SaaS — currently the roadmap is Western-payments-only |
...
| RAG helper (auto-chunk/embed content on model save) | 🆕 | Medium | L | `MODEL`, `HOOKS` | Drop-in "chat with your data" for any model, given how AI-heavy Mudia's own project portfolio already is |

### K. Local-First & Cross-Platform 🆕 *(entirely new category)*

| Feature | Status | Value | Effort | Primitives | Unlocks |
|---|---|---|---|---|---|
| `defineLocalModel()` — Dexie-backed local storage + sync-when-online + conflict resolution | 🆕 | High | XL | `MODEL`, `RF` | Promotes the pattern already hand-built in Lowo into a reusable primitive; offline-capable apps by default |
| `omni add tauri` / `omni add capacitor` scaffolding | 🆕 | High | M | `CLI` | "Build anything with web tech" literally extends to desktop/mobile, matching patterns already used in StudyScroll/CodeMentor AI |
...
| Nigerian phone-number/NIN validation helpers | 🆕 | Medium | XS | `SCHEMA` | Small effort, high signal that the framework is built *for* this market |
| Feature flags module | 🆕 | Medium | M | `PLUGIN`, `CACHE` | Safe rollout/kill-switches without a third-party flag service |
| i18n/localization module | 🆕 | Medium | M | `SCHEMA`, `RF` | Multi-language apps — frequently a real requirement, not a nice-to-have, in African markets |
...
| Feature | Status | Value | Effort | Primitives | Unlocks |
|---|---|---|---|---|---|
| `OmniPlugin` lifecycle interface | 🔧 *(broken import, see §0)* | High | XS *(fix)* | `PLUGIN` | Blocks literally everything else in this section until fixed |
...
| **v0.8 — Plugin Ecosystem** | Planned | Q3 2027 | **Fix `OmniPlugin` import bug first** (blocks everything); plugin-contributed remote functions |
...
| **v1.3 — Global Business Primitives** 🆕 | New phase | 2028+ | `Money` type, `defineWorkflow()`, African SMS gateways, Nigerian ID/phone validation |

---

## 4. Prioritization methodology

Value was scored by how directly a feature removes *repetitive, non-business-logic* work (the stated mission), not by novelty. Effort was scored by how much net-new design is required versus how much can be assembled from primitives that already exist. Two things consistently pushed a feature up in priority regardless of effort: (1) it fixes something broken rather than something merely missing (§0's plugin bug), and (2) it's a primitive other planned features depend on (e.g. `resource()` and `query.live` unlock several other rows in these tables, so they're worth doing early even though they're not the cheapest items).

## 5. Risks & dependencies

- **Remote functions are experimental upstream.** SvelteKit explicitly flags `query`/`form`/`command`/`query.live` as subject to breaking changes without notice. Building core framework primitives on top of an unstable API is a real bet — worth it for the leverage, but track upstream changelog closely and isolate the integration behind OmniSvelte's own API so a breaking upstream change doesn't ripple into every app built on the framework.
- **Plugin ecosystem has a cold-start problem.** A marketplace and community plugins are worthless without a working `OmniPlugin` interface and a few flagship first-party plugins to prove the pattern — sequence v0.8 accordingly.
- **Bus factor.** This is currently a solo/small-team effort alongside academic work and competitions (XPRIZE, hackathons). The roadmap above is intentionally sequenced so that *fixing* and *finishing* (v0.2–v0.9) come before *expanding* (v1.1+), so the framework is usable and credible even if later phases slip.
...
- **Differentiation vs. crowded competition** (Next.js, Remix, Laravel, SvelteKit alone) — the AI-native and Africa-market primitives are the parts of this roadmap with the least direct competition; they're worth protecting from scope creep into "yet another generic full-stack framework."
...
- Number of production apps built on OmniSvelte (your own — Lowo, ReelView, LexAI, the football game backend — count as the first cohort).
