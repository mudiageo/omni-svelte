...
import { hashPassword, generateSlug } from '../../utils/index.js';
import type { Schema, GeneratedOutput } from '../types.js';
import type { generateDrizzleSchema } from './drizzle.js';
import type { generateZodSchemas } from './zod.js';
...
function determineCasts(
	schema: Schema
): Record<string, 'string' | 'number' | 'boolean' | 'date' | 'json'> {
...
export class ModelGenerator {
	...
	private generateCombinedModels(
		schemas: Schema[],
		outputConfig: any
	): { imports: string; models: string } {
		...
		consolidatedImports.push(
			`import { Model } from '${pathResolver.toImportPath(pathResolver.getModelBaseImportPath('model'))}';`
		);
		...
		if (drizzleImports.size > 0) {
			...
			consolidatedImports.push(
				`import { ${Array.from(drizzleImports).sort().join(', ')} } from '${pathResolver.toImportPath(drizzleImportPath)}';`
			);
		}
		...
		validationImports.forEach((imports, schemaName) => {
			if (imports.size > 0) {
				...
				consolidatedImports.push(
					`import { ${Array.from(imports).sort().join(', ')} } from '${pathResolver.toImportPath(zodImportPath)}';`
				);
			}
		});
		...
		if (typeImports.size > 0) {
			consolidatedImports.push(
				`import type { ${Array.from(typeImports).sort().join(', ')} } from '${pathResolver.toImportPath(pathResolver.getDrizzleImportPath(modelPath))}';`
			);
		}
...
import type { ${capitalizedName} as ${capitalizedName}Type, New${capitalizedName} as New${capitalizedName}Type } from '${this.pathResolver.toImportPath(imports.drizzle)}';`;
		...
		const modelImport = `import { Model } from 'omni-svelte/model';`;
