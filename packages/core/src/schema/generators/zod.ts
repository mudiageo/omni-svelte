...
function generateZodField(field: FieldDefinition): z.ZodTypeAny | null {
	...
	switch (field.type) {
		...
		case 'datetime':
			...
			if (field.validation?.min) {
				const minDate =
					field.validation.min === 'today' ? new Date() : new Date(field.validation.min as string);
				zodField = (zodField as z.ZodDate).min(minDate);
			}
			if (field.validation?.max) {
				const maxDate =
					field.validation.max === 'today' ? new Date() : new Date(field.validation.max as string);
	...
	if (field.validation?.min && ['string', 'email', 'password'].includes(field.type)) {
		zodField = (zodField as z.ZodString).min(
		...
		if (outputConfig.format === 'single-file') {
			...
			const allSchemas = schemas
				.map((schema) => {
					...
					return content.replace(/^import { z } from 'omni\-svelte\/validation';\s*\n/m, '');
				})
				.join('\n\n');
