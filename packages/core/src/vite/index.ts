...
const plugin_omni_virtual_aliases: Plugin = {
  ...
  async config(userConfig) {
    ...
    const modelsPath = out?.model?.path ? resolve(root, out.model.path) : resolve(root, 'src/lib/db/models');
    const drizzlePath = out?.drizzle?.path ? resolve(root, out.drizzle.path) : resolve(root, 'src/lib/db/server/schema.ts');
    const zodPath = out?.zod?.path ? resolve(root, out.zod.path) : resolve(root, 'src/lib/db/validation');
  ...
  async configResolved(resolvedConfig) {
    ...
    const modelsAbsPath = out?.model?.path ? resolve(root, out.model.path) : resolve(root, 'src/lib/db/models');
    const modelsFormat = out?.model?.format ?? 'per-schema';
    const drizzleAbsPath = out?.drizzle?.path ? resolve(root, out.drizzle.path) : resolve(root, 'src/lib/db/server/schema.ts');
    const zodAbsPath = out?.zod?.path ? resolve(root, out.zod.path) : resolve(root, 'src/lib/db/validation');
  ...
  async load(id) {
    ...
    if (key === 'models') {
      ...
      if (existsSync(modelsPath)) {
        ...
        const files = readdirSync(modelsPath).filter(f => f.endsWith('.model.ts') || f.endsWith('.model.js'));
        const exports = files.map(f => `export * from '${(modelsPath + '/' + f).replace(/\\/g, '/')}';`).join('\n');
    ...
    if (key === 'validation') {
      ...
      if (existsSync(zodPath)) {
        const { readdirSync } = await import('fs');
        const files = readdirSync(zodPath).filter(f => f.endsWith('.validation.ts') || f.endsWith('.validation.js') || f.endsWith('.ts') || f.endsWith('.js'));
        const exports = files.map(f => `export * from '${(zodPath + '/' + f).replace(/\\/g, '/')}';`).join('\n');
