import type { GeneratedOutput, Schema, SchemaConfig, UserSchemaConfig } from '../schema/types.js';
import type { OmniConfig, SvelteConfig } from '../config/types.js';
...
import { ParserFactory } from '../schema/parser.js';
import { writeFile, mkdir } from 'fs/promises';
import { dirname } from 'path';
import { syncAuthSchemas } from '../runtime/auth/auth-sync.js';
...
export async function initializeSchemaConfig(omniConfig: OmniConfig, root: string) {
	const userSchemaConfig: UserSchemaConfig =
		omniConfig?.schema || (await loadSchemaConfigFile(root));
...
async function loadSchemaConfigFile(root: string): Promise<UserSchemaConfig> {
	const configFiles = ['schema.config.ts', 'schema.config.js'];
			...
			for (const schema of fileSchemas) {
				...
				if (existingSchema) {
					if (config.dev?.logLevel !== 'silent') {
						console.warn(
							`⚠️  Duplicate schema name "${schema.name}" found in ${file}. Skipping...`
						);
					}
					continue;
				}
				...
				if (config.dev?.logLevel !== 'silent') {
					console.log(
						`✅ Found schema: ${schema.name} (${Object.keys(schema.fields).length} fields)`
					);
				}
			}
		} catch (error) {
