...
describe('Path Resolution - Final Validation', () => {
  it('should correctly resolve paths for library usage with omni-svelte package imports', () => {
    const config = {
    ...
    // Test Model base import - should use omni-svelte package in generated output
    const modelImports = resolver.resolveModelImports('./src/lib/models/users.model.ts', 'users');
    expect(modelImports.modelBase).toBe('omni-svelte/model');
