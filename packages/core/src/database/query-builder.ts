...
export class QueryBuilder<T extends typeof Model> {
	...
	search(
		...
		const { caseSensitive = false, wildcards = true, exact = false } = options;
	...
	async get(): Promise<InstanceType<T>[]> {
		...
		const models: InstanceType<T>[] = results.map((row: Record<string, any>) =>
			this.newFromBuilder(row)
		);
	...
	// Aggregates
	async aggregate(
		aggregates: Record<string, { fn: 'count' | 'sum' | 'avg' | 'min' | 'max'; column?: string }>
	) {
		...
		const result = await getDatabase().select(selectObj).from(this.modelClass.table);
	...
	async paginate(perPage = 15, page = 1) {
		...
		const countQuery = getDatabase().select({ count: count() }).from(this.modelClass.table);
	...
	async chunk(size: number, callback: (models: InstanceType<T>[]) => Promise<void> | void) {
		...
		while (hasMore) {
			const results = await this.offset((page - 1) * size)
				.limit(size)
				.get();
