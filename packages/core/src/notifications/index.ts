...
const channelHandlers = new Map<string, (notification: Notification, recipient: NotificationRecipient) => Promise<void>>();

let notificationConfig: NotificationConfig = {
    channels: [
        { name: 'database', driver: 'database' },
    ],
...
export async function notify(
    ...
    for (const channel of notification.channels) {
        ...
        if (handler) {
            ...
            console.warn(`🔔 [Notifications] No handler for channel '${channel}'. Notification stored but not delivered.`);
        }
    }
