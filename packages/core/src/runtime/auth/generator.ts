...
export class AuthConfigGenerator {
	async generate(authConfig: AuthConfig) {
		...
		await Promise.all([this.generateAuthConfig(authConfig), this.generateClientConfig(authConfig)]);
	}
...
`;
	...
	private generateAccountConfig(config?: AuthConfig['account']) {
		...
		if (config.accountLinking) {
			...
			if (config.accountLinking.trustedProviders) {
				configLines.push(
					`    trustedProviders: [${config.accountLinking.trustedProviders.map((p) => `'${p}'`).join(', ')}],`
				);
			}
			configLines.push(`  },`);
		}
	...
	private generateAdvancedConfig(config?: AuthConfig['advanced']) {
		...
		if (config?.defaultCookieAttributes) {
			configLines.push(
				`  defaultCookieAttributes: ${JSON.stringify(config.defaultCookieAttributes, null, 2)},`
			);
		}
    ...
    config: ${JSON.stringify(cfg?.config || [], null, 2)},
		...
		for (const [pluginName, pluginConfig] of Object.entries(
			plugins as Record<string, PluginValue>
		)) {
...
export default generatedClientConfig;
    `;
		...
		write_if_changed(`${runtime_directory}/auth/__generated__/client-config.js`, configCode);
	}
}
