...
export class AuthSchemaSync {
	...
	private async updateUserOmniSchema(
		...
		const { path: userSchemaPath, tableName: userTableName } =
			this.findUserSchemaFile(existingSchemas);
		...
		const schemaMatch = schemaContent.match(
			new RegExp(`defineSchema\\s*\\(\\s*['"]${userTableName}['"]\\s*,\\s*\\{`)
		);
	...
	private async generateAuthOmniSchemaFile(
		...
		code += `import { defineSchema } from 'omni-svelte/schema';\n\n`;
	 ...
	 */
	private findUserSchemaFile(existingSchemas: Schema[]): {
		path: string | null;
		tableName: string;
	} {
		// Try to find from discovered schemas (checks both 'user' and 'users')
		const userSchema = existingSchemas.find(
			(schema) => schema.name === 'user' || schema.name === 'users'
		);
