...
export function hashAuthConfig(config: any): string {
	...
	return crypto.createHash('sha256').update(JSON.stringify(relevantConfig)).digest('hex');
}

/**
 * Check if schema needs regeneration based on config changes
 */
export function shouldRegenerateSchema(projectRoot: string, configHash: string): boolean {
	...
	const hasSQLiteTimestamp =
		...
		Object.values(tables).some((table: any) =>
			Object.values(table.fields).some(
				(field: any) =>
					field.type === 'date' && field.defaultValue && typeof field.defaultValue === 'function'
			)
		);
